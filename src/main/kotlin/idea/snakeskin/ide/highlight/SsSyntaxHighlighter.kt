...
class SsSyntaxHighlighter : SyntaxHighlighterBase() {
	companion object {
		fun map(tokenType: IElementType): SsColor? = when (tokenType) {
			...
			XML_IDENTIFIER, XML_IDENTIFIER_PART_START, XML_IDENTIFIER_PART_END -> SsColor.XML_NAME
			CLASS_SELECTOR, CLASS_SELECTOR_PART_START, CLASS_SELECTOR_PART_END -> SsColor.CLASS_NAME
			ID_SELECTOR, ID_SELECTOR_PART_START, ID_SELECTOR_PART_END -> SsColor.ID_NAME
